...
2. **Authentication** → **URL configuration**:
   - **Site URL**: `http://localhost:3000` (or the port shown when you run `npm run dev`)
   - **Redirect URLs**: add your app callback, e.g. `http://localhost:3000/auth/callback` and any extra ports (`3005`, etc.)

### Step 3 — Local port

If `npm run dev` says `using port 3005`, either:

- Add `http://localhost:3005/auth/callback` to Supabase **Redirect URLs**, and set in `.env.local`:
...
### “Unable to exchange external code” (hash or `auth_error=exchange`)
...
5. Confirm Supabase **Redirect URLs** includes your dev URL, e.g. `http://localhost:3005/auth/callback`.
...
Open [http://localhost:3000](http://localhost:3000) for the landing page. Use **Google** on the hero to sign in, or **Login** in the navbar for Google and email.
